...
d_smd_wt = dta_c_panel %>%
  # End of grace period
  dplyr::filter(time==12 & censor==0) %>%
  summarize(`X, mean (SD)` = (weighted.mean(X[assign==1], w = marg_ipw[assign==1]) - 
                     weighted.mean(X[assign==0], w = marg_ipw[assign==0])) / sd(X),
            `Xt, mean (SD)`  = (weighted.mean(X_t[assign==1], w = marg_ipw[assign==1]) - 
                      weighted.mean(X_t[assign==0], w = marg_ipw[assign==0])) / sd(X_t),
            `Age, years (SD)` = (weighted.mean(age[assign==1], w = marg_ipw[assign==1]) - 
                               weighted.mean(age[assign==0], w = marg_ipw[assign==0])) / sd(age),
            `Sex, female` = (weighted.mean(female[assign==1], w = marg_ipw[assign==1]) - 
                                  weighted.mean(female[assign==0], w = marg_ipw[assign==0])) / sd(female)
         ) %>%
  t() %>%
  as_tibble(rownames = 'Variable') %>%
  rename(`IPW aSMD` = V1)
